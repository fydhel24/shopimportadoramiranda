...
import styles from "../../pedido/steps/form.module.css";
...
const departamentos = {
  "Santa Cruz": ["Santa Cruz", "Montero", "Camini"],
...
const Paso1 = ({ formData, onChange, formSubmitted }: Paso1Props) => {
  ...
  const [modalShownOnce, setModalShownOnce] = React.useState(false); // evita que se muestre múltiples veces
  ...
  return (
    <div>
      ...
      >
        ...
        />
        {hasError("nombre") && (
          <p className={styles.errorMessage}>⚠ Por favor, ingrese su nombre.</p>
        )}
      </div>
            ...
            {provincias.map((prov) => (
              <button
                ...
                className={`${styles.provinciaBtn} ${
                  formData.provincia === prov ? styles.provinciaSelected : ""
                }`}
              >
                {prov}
              </button>
