...
const Formulario = () => {
  ...
  const sendOrderToAPI = async () => {
    try {
      ...
      apiFormData.append("id_sucursal", "2");
      ...
      generatePDF(pedidoNumero); // Generar PDF
      handleRedirectToWhatsApp(pedidoNumero); // Redirigir a WhatsApp
    } catch (error) {
  ...
  const generatePDF = (pedidoMessage: string): void => {
    ...
    doc.setFillColor(128, 0, 128); // Morado
